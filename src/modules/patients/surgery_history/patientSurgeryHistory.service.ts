...
import { PatientSurgeryHistoryRepository } from './patientSurgeryHistory.repository';
import { SyncPatientSurgeryHistoryPayload } from './patientSurgeryHistory.types';

export class PatientSurgeryHistoryService {
  private repo = new PatientSurgeryHistoryRepository();
  ...
  async syncBulk(patientId: number, payloads: SyncPatientSurgeryHistoryPayload[], userId?: string) {
    await this.ensurePatientActive(patientId);
    
    // 1. Get current active surgical history for patient
    ...
    for (const item of toUpdate) {
      ...
      if (existing) {
        await this.repo.update(existing.id, {
          surgery: { connect: { surgery_id: item.surgeryId ?? existing.surgery_id } },
