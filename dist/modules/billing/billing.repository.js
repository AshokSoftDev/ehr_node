...
exports.billingRepository = {
    ...
    async deleteInvoiceItems(invoiceId) {
        return prisma_1.prisma.invoiceItem.deleteMany({
            where: { invoice_id: invoiceId },
        });
    },
    async createInvoiceItems(items) {
        return prisma_1.prisma.invoiceItem.createMany({
            data: items,
        });
    },
    // Receipt operations
    async createReceipt(data) {
        return prisma_1.prisma.receipt.create({
            data,
            include: {
                invoice: {
                    select: {
    ...
    async findReceiptById(id) {
        return prisma_1.prisma.receipt.findUnique({
            where: { receipt_id: id },
            include: {
                invoice: {
                    select: {
    ...
    async updateReceipt(id, data) {
        return prisma_1.prisma.receipt.update({
            where: { receipt_id: id },
            data,
            include: {
                invoice: {
