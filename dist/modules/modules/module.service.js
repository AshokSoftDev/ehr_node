...
class ModuleService {
    ...
    async seedModules() {
        ...
        for (const moduleData of this.defaultModules) {
            ...
            if (!module) {
                // Create module
                module = await prisma_1.prisma.module.create({
                    data: {
                ...
                if (moduleData.subModules && moduleData.subModules.length > 0) {
                    await prisma_1.prisma.subModule.createMany({
    ...
    async addModule(dto) {
        ...
        if (existingModule) {
            throw new errors_1.AppError('Module with this name already exists', 409);
        }
    ...
    async addSubModule(moduleId, dto) {
        ...
        const existingSubModule = await prisma_1.prisma.subModule.findFirst({
            where: {
                moduleId,
                name: dto.name,
            },
        ...
        if (existingSubModule) {
            throw new errors_1.AppError('SubModule with this name already exists for this module', 409);
        }
        return prisma_1.prisma.subModule.create({
            data: {
                ...dto,
                moduleId,
            },
